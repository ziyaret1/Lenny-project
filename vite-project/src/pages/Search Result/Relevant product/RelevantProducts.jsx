...
const RelevantProducts = () => {
  ...
  const { categoryProd, currentPage, filterObj } = useSelector(
  ...
  return (
    <div className="relevantProducts-container">
      <div className="relevantProductList">
        {categoryProd?.data?.map((product) => {
          return (
            ...
            >
              <ProductCard
                prodImage={product?.attributes?.image?.data[0]?.attributes?.url}
      ...
      <div className="relevantProductPagination">
        <Stack spacing={2}>
          <Pagination
            count={categoryProd?.meta?.pagination?.pageCount}
